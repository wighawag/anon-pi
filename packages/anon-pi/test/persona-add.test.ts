...
describe('planPersonaAdd: resumable across account creation', () => {
	it('account MISSING -> wait-for-account with the Tier-2 command block + instruction', () => {
		...
		expect(plan.script).toContain('useradd -m anonpi-alice');
		expect(plan.script).toContain('loginctl enable-linger anonpi-alice');
	...
	it('already fully provisioned -> already-provisioned (idempotent no-op re-run)', () => {
		const plan = planPersonaAdd({
			...
			accountExists: true,
			anonHome: '/home/anonpi-alice/.anon-pi',
			config: {proxy: composeTorPersonaProxy('anonpi-alice')},
	...
	it('the default persona uses account `anonpi` (empty-suffix), no special-casing', () => {
		const plan = planPersonaAdd({
		...
		expect(plan.kind).toBe('wait-for-account');
		if (plan.kind !== 'wait-for-account') throw new Error('unreachable');
		expect(plan.script).toContain('useradd -m anonpi');
	});
});
...
describe('buildPersonaTeardownScript: the root teardown block for `persona rm`', () => {
	it('emits the ordered teardown for a named persona (sudoers, linger, userdel -r)', () => {
		...
		expect(script).toContain("rm -f '/etc/sudoers.d/anon-pi-anonpi-alice'");
		// disable linger, then delete the account + home.
		expect(script).toContain('loginctl disable-linger anonpi-alice');
		expect(script).toContain('userdel -r anonpi-alice');
	...
	it('targets the default `anonpi` account for a bare teardown', () => {
		const script = buildPersonaTeardownScript(ANON_ACCOUNT);
		expect(script).toContain('userdel -r anonpi');
		expect(script).toContain("rm -f '/etc/sudoers.d/anon-pi-anonpi'");
	});
});
