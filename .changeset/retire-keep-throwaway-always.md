...
container into a named image (`anon-pi machine snapshot <name>`) and pin a
machine to it (`anon-pi machine create <m> --image <name>`). The exploratory
...
`forward`, `ports`, and `machine snapshot` are unchanged: anon-pi still stamps
