...
All decisions are settled; `needsAnswers` is cleared and the spec is ready to task. The v1 scope is DELIBERATELY narrowed to init-driven hardening (see below); the standalone `harden` verb and its workspace-import step are deferred to the idea `work/notes/ideas/harden-command-with-import.md`.
...
7. **Changeset + verify gate applies.** anon-pi's `.dorfl.json` verify is `pnpm format:check && pnpm changeset status --since=main && pnpm -r build && pnpm -r test`; every task emitted from this spec adds a `pnpm changeset` and stays green under it (unlike netcage, this repo really does use changesets).
...
- **netcage changes.** The netcage side is the uid-scoped-store fix, owned in the netcage repo (ADR-0017 / prd `uid-scoped-graphroot-multi-user-fix`). This spec DEPENDS on a netcage version that has it; it does not modify netcage. In particular anon-pi does NOT set `NETCAGE_GRAPHROOT` (the uid-scoped default suffices).
...
- **This spec supersedes the idea note `work/notes/ideas/hardened-dedicated-account-deployment.md`** (same slug). That note should be updated or retired once this spec is tasked. It is largely correct, but its netcage dependency was framed WRONG and must not be carried forward verbatim (see below); also, its `anon` wrapper is superseded by self-re-exec.
