...
**This task OWNS retiring the legacy pure-module surface it replaces** (the
...
`resolveConfigSeed`, and the now-dead per-workdir container-path constants, and
rewrite/retire the pure-module tests locked to that model in `anon-pi.test.ts`
(the old `buildRunPlan`/`stateAgentDir`/`resolveConfigSeed` cases). Also perform
the container-path CONSTANT rename in the pure module (`CONTAINER_WORKDIR` +
related → `/projects`, plus the distinct `--mount` `/work`), so the later
`images-projects-path-rename` task only edits the Dockerfiles + `trust.json`.
(The `import`-source pure logic — `pickProviderForLlm`/`resolveSourceModelsPath`
— is retired by `models-json-generation-from-llm`, not here; the `HELP` string
and the `--fresh`/`--ephemeral`/`import` CLI surface are retired by
...
- [ ] The legacy pure-module surface this replaces is RETIRED, not left dead:
      ...
      the `anon-pi.test.ts` cases locked to them are rewritten or deleted (no
      red tests remain).
...
You OWN retiring the pure-module surface this replaces (verify runs `pnpm -r
test`, so nothing may stay red): remove/replace the old `buildRunPlan`,
`stateAgentDir`, `resolveConfigSeed`, and the dead per-workdir container-path
constants, and rewrite or delete the `anon-pi.test.ts` cases pinned to them. Do
the container-path CONSTANT rename here too (`CONTAINER_WORKDIR`/related →
`/projects`; distinct `--mount` `/work`), so the images task only edits the
Dockerfiles + `trust.json`. Leave the `import`-source logic
(`pickProviderForLlm`/`resolveSourceModelsPath`) to
`models-json-generation-from-llm` and the `HELP`/`--fresh`/`--ephemeral`/`import`
CLI surface to `cli-launch-surface-grammar-a`.
