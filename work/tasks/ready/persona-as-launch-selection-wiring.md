...
- **Preflight for the selected persona.** v1's preflight checked the `anon` account; make it check the SELECTED persona account. The subid check now verifies a range EXISTS for the account (not a specific start — decision 0's auto-allocation). netcage `>= 0.11.0` floor unchanged.
...
- [ ] The preflight targets the selected persona account; the subid check verifies a range EXISTS (not a specific start), netcage floor `>= 0.11.0` unchanged.
